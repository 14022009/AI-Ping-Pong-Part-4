...
function setup() {
  ...
  video.parent("canvas");
  video.hide();
  pose = ml5.poseNet(video, modalloaded);
...
function gotPoses(result) {
  if (result.length > 0) {
    ...
    right_wrist_Y = result[0].pose.rightWrist.x;
    console.log(result);
    right_wrist_S = result[0].pose.keypoints[10].score;
  }
}
...
function draw() {
  ...
  stroke("black");
  rect(680, 0, 20, 700);
  console.log(right_wrist_S);
  if (right_wrist_S > 0.2) {
    fill("red");
    stroke("red");
